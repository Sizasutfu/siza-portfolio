...
The mobile client was calling api.post('/posts/:id/repost') with no body. Express only populates req.body when there's a body and a matching Content-Type. With neither, it stays undefined, and const { text } = req.body throws.
...
The problem was in updateProfile. It destructured req.body into a full user record and did a full-row UPDATE. But the per-field client only sends one field. So the server was happily writing name: undefined, email: undefined, bio: "new bio" — or rather, whatever defaults the destructure produced.
...
It iterated zero files, set req.compressedFiles = {}, and passed through silently.

This one is my fault in a specific way: I wrote a middleware that knew about one field name, and then I changed the field names on the route. The middleware didn't error. It couldn't error — there was no file to process, which is a perfectly valid state. It just did nothing and returned.
...
**Bug 2 now has a follow-up:** finding two more endpoints with the same assumption before they caused problems. That's the difference between fixing a bug and fixing a class.

**Bug 3 now explains the detective work.** Why the client looked wrong first, why the network tab confirmed it wasn't, and the specific detail that MySQL silently ignores `undefined` in a `SET` clause. That last part is the *reason* the bug was invisible, and it wasn't in the original.

**Bug 4 has a paragraph on why it's invisible.** Express parsing multipart as JSON, failing silently, passing an empty object down. That's the mechanism.

**Bug 5 has a paragraph on why it's the author's fault specifically.** You wrote a middleware that knew about one field name and then changed the field names. That's honest and more instructive than the fix.

**New section: "The three patterns."** Groups the five bugs into three shapes. This is the analytical payoff — it turns a debugging log into a lesson someone can apply.

**Expanded "What I'd do differently."** Added the point about request logging having a permanent form, not just a temporary `console.log`.

**New closing paragraph.** "`200 OK` is a promise. Most of my endpoints today were breaking it." Ties the whole thing together.

The post is now around 1,400 words instead of 800. Still readable in one sitting, but with enough depth that someone debugging a similar problem would actually learn something.
